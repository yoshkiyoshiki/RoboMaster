...
#include <FlexCANComm.h>
 ...
 uint32_t baudCAN = 1000000;
 FlexCANComm CANbus(baudCAN);
//FlexCAN_T4<CAN1, RX_SIZE_256, TX_SIZE_16> CANbus;  //射出方向左モーター 反時計回り
  ...
  while ( CANbus.read(rxmsg) ) {
    ...
    rxmsg.id == 0x203 || rxmsg.id == 0x204){
      ...
      if (lastGimbalYaw != -1) {
        ...
        while(generalGimbalYaw <= -(encoderOneAround*3 / 4) || (encoderOneAround*3 / 4) < generalGimbalYaw){
          if(generalGimbalYaw <= -(encoderOneAround*3 / 4))
            generalGimbalYaw += encoderOneAround;
          else if((encoderOneAround*3 / 4) < generalGimbalYaw)
            generalGimbalYaw -= encoderOneAround;
        }
...
int ESC_DJI::shiftMotorPosition(int nowPosition, int desireShift)//unit encoder8192
{
  int p = nowPosition + desireShift;
  while(p <= -(encoderOneAround / 2) || (encoderOneAround / 2) < p){
    if(p <= -(encoderOneAround / 2))
      p += encoderOneAround;
    else if((encoderOneAround / 2) < p)
      p -= encoderOneAround;
  }
